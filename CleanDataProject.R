...
features <- unlist(strsplit(as.character(w, split= "", fixed= TRUE))
#load train group text file
traindata <- read.delim("./data/X_train.txt", sep="", header=FALSE)
#rename variable columns to features second column
colnames(traindata) <- features$V2
#load numeric value activity lables for train data
traindatalabels <- read.delim("./data/y_train.txt", sep="", header=FALSE)
...
testdata <- read.delim("./data/X_test.txt", sep="", header=FALSE)
#rename variable columns to features second column
colnames(testdata) <- features$V2
#load numeric value activity lables for test data
testdatalabels <- read.delim("./data/y_test.txt", sep="", header=FALSE)
